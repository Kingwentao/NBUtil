...
class PermissionsUtil private constructor(private val permissions: Array<out String>) {
     ...
     */
    fun rational(rational: PermissionRational?): PermissionsUtil {
     ...
     * 可在此提醒用户前往权限设置。打开权限。
     */
    fun alwaysDenyNotifier(denied: PermissionDenied?): PermissionsUtil {
        this.denied = denied
        return this
    }
     ...
     * 设置授权结果回调
     */
    fun callback(callback: PermissionCallback?): PermissionsUtil {
    ...
    companion object {
        ...
        @JvmStatic
        fun create(vararg permissions: String): PermissionsUtil {
            return PermissionsUtil(permissions)
        }
        ...
        fun isPermissionsGranted(activity: Activity, vararg permissions: String): Boolean {
            for (permission in permissions) {
                if (!isPermissionGranted(permission, activity)) return false
            }
            return true
        }
        ...
        fun getPermissionGroupInfos(permissions: Array<String>, context: Context): List<PermissionGroupInfo> {
            ...
            for (permission in permissions) {
                val group = context.packageManager.getPermissionInfo(
                        permission, PackageManager.GET_META_DATA)?.group
                ...
                val groupInfo = context.packageManager.getPermissionGroupInfo(
                        group,
...
@TargetApi(Build.VERSION_CODES.M)
class RationalChain internal constructor(
        private val denies: MutableIterator<String>,
...
class PermissionFragment : Fragment() {
    ...
    private var callback: PermissionCallback? = null
    private var denied: PermissionDenied? = null
    private var request: PermissionsUtil? = null
...
abstract class PermissionAlwaysDenyNotifier {
    ...
    fun goSetting(activity: Activity) =
            PermissionFragment.findOrCreate(activity).goSetting(createIntent(activity))
